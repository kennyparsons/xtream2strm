...
func GetSeries(config models.Config) error {
	...
	if err != nil {
		return fmt.Errorf("failed to fetch series data from Xtream Codes API: %w", err)
	}
	defer resp.Body.Close()

	// Check the status code of the response
	if resp.StatusCode != http.StatusOK {
		return fmt.Errorf("API request for series failed with status code %d", resp.StatusCode)
	}

	// Read the response body
	body, err := ioutil.ReadAll(resp.Body)
	if err != nil {
		return fmt.Errorf("failed to read series response body: %w", err)
	}
	...
	if err != nil {
		return fmt.Errorf("failed to parse series JSON response: %w", err)
	}
